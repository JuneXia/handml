...
W_conv = {
    ...
    'fc2': tf.Variable(tf.truncated_normal([n_fc2, n_fc3], stddev=0.1)),
    'output': tf.Variable(tf.truncated_normal([n_fc3, n_classes], stddev=0.1))
}
...
# LRN层
pool1 = tf.nn.lrn(pool1, 5, bias=1.0, alpha=0.001/9.0, beta=0.75)
...
# LRN层
pool2 = tf.nn.lrn(pool2, 5, bias=1.0, alpha=0.001/9.0, beta=0.75)
...
# 损失函数
# cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(y_output, y))  # failed

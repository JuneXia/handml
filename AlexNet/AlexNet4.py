...
# 保存loss和测试准确率到csv文件
with open('AlexNet-input?x227x227x1-conv?x55x55x96lrn-maxpool?x27x27x96-conv?x27x27x256lrn-maxpool?x13x13x256-conv?x13x13x384-conv?x13x13x384-conv?x13x13x256-maxpool?x6x6x256-fc?x4096-fc?x4096-output?x10-learnrate_1e-4-tf_global_init-epoch50.csv', 'w') as fid:
...
'''
使用tf.get_variable创建变量，使用tf.global_variables_initializer初始化
